...
import com.mindlinksw.schoolmeals.databinding.VideoDetailActivityBinding;
...
import com.mindlinksw.schoolmeals.view.fragment.VideoFrameFragment;
...
public class VideoDetailActivity extends BaseActivity implements Initialize, SwipeRefreshLayout.OnRefreshListener {

    private final String TAG = VideoDetailActivity.class.getName();

    private VideoDetailActivityBinding mBinding;
    ...
    private VideoFrameFragment mVideoFrameFragment;


    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        mBinding = DataBindingUtil.setContentView(this, R.layout.video_detail_activity);
    ...
    @Override
    public void onPause() {
        if (mVideoFrameFragment != null) {
            mVideoFrameFragment.onPause();
        }
        super.onPause();
    }
    ...
    public void onBackPressed() {
        // Full ScreenCheck
        if (mVideoFrameFragment.isFullScreen()) {
            mVideoFrameFragment.setFullScreen(false);
        } else {
            super.onBackPressed();
        }
    }
        ...
        try {
            ...
            if (!mIsDelete) {
                ...
                if (!ObjectUtils.isEmpty(detail)) {
                    ...
                    // 썸내일
                    mBoardModel.setThumbnailUrl1(detail.getThumbnailUrl());
    ...
    public void initLayout(@Nullable View view) {
        ...
        // video fragment
        mVideoFrameFragment = new VideoFrameFragment();
        FragmentUtils.addFragmentNoBackStack(getSupportFragmentManager(), R.id.fl_video, mVideoFrameFragment);
    ...
    public void initEventListener() {
        recordAnalytics("VedioDetailActivity","VedioDetailActivityView","activity");
    }
    ...
    public class ViewModel {
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                ...
                case R.id.tv_comment_submit:
                    // 댓글 등록
                    if (SessionSingleton.getInstance(VideoDetailActivity.this).isExist()) {
                        setComment(mBoardId, mBinding.etComment1dept.getText().toString());
                    } else {
                        SnackBarUtils.login(VideoDetailActivity.this);
                    }
                    break;
                        ...
                        public void onDismiss(int code) {
                            switch (code) {
                                case DetailBottomSheet.CODE.MODIFY:
                                    // 수정
                                    Intent intent = new Intent(VideoDetailActivity.this, VideoWriteActivity.class);
                                ...
                                case DetailBottomSheet.CODE.DELETE:
                                    // 삭제
                                    new AlertDialog(VideoDetailActivity.this)
        ...
        public void onClick(View v, BoardData model) {
            ...
            try {
                ...
                switch (v.getId()) {
                    ...
                    case R.id.tv_prev: // 이전글
                        ...
                        Intent prevIntent = new Intent(VideoDetailActivity.this, BridgeActivity.class);
                        prevIntent.putExtra("type", CategoryConst.VIDEO);
                    ...
                    case R.id.tv_next: // 다음글
                        ...
                        Intent nextIntent = new Intent(VideoDetailActivity.this, BridgeActivity.class);
                        nextIntent.putExtra("type", CategoryConst.VIDEO);
        ...
        public void getDetail() {
            ...
            SchoolMealsService service = RetrofitRequest.createRetrofitJSONService(VideoDetailActivity.this, SchoolMealsService.class, HostConst.apiHost());
            Call<BoardData> call = service.reqVideoDetail(mBoardId);

            RetrofitCall.enqueueWithRetry(call, new Callback<BoardData>() {
                ...
                public void onResponse(Call<BoardData> call, Response<BoardData> response) {
                    ...
                    try {
                        ...
                        if (response.isSuccessful()) {
                            ...
                            BoardData resData = (BoardData) response.body();
                            ...
                            mModel.set(resData);
                            mLikeCount.set(resData.getRecommendCnt());

                            // 자기가 등록한 글인지 확인
                            mIsSelf.set(SessionSingleton.getInstance(VideoDetailActivity.this).isSelf(resData.getEsntlId()));
                             ...
                             * youtube
                             */
                            mVideoFrameFragment.initYoutube(resData);
                            ...
                            if (resData != null) {
                                if ("success".equals(resData.getCode())) {
                                    ...
                                    runOnUiThread(new Runnable() {
                                        @Override
                                        public void run() {
                                            Utils.setKeyboardShowHide(VideoDetailActivity.this, mBinding.getRoot(), false);
                                        }
                                    });
        ...
        public void setLike(final CheckBox checkBox, final BoardData model) {
            ...
            BoardAPI.setLike(VideoDetailActivity.this, checkBox.isChecked(), model.getBoardId(), new OnResponseListener() {
                @Override
        ...
        public void setBookmark(final CheckBox checkBox, final BoardData model) {
            ...
            BoardAPI.setBookmark(VideoDetailActivity.this, checkBox.isChecked(), model.getBoardId(), new OnResponseListener() {
                @Override
                public void onSuccess() {
